            ...
            .then(([rows]) => {
              ...
              prompt([
                ...
                .then((role) => {
                  ...
                  // Check to see if assigned role matches current role, if it does, the program will exit with a descriptive error message.
                  if (employee.employeeSelection === role.roleSelection) {
                    console.error(`Employee cannot be assigned the same role as they already have, error encountered, shutting down.`)
                    process.exit(0);
                  }

                  db.promise().query(sql, [role.roleSelection, employee.employeeSelection])
                    .then(() => {
